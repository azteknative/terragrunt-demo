# Local values
locals {
  cluster_name = var.cluster_name != null ? var.cluster_name : "${var.project_name}-${var.environment}"
...
# Additional policy for Systems Manager Parameter Store (if secrets are used)
resource "aws_iam_role_policy" "ecs_task_execution_role_ssm_policy" {
  count = length(var.secrets) > 0 ? 1 : 0
  name  = "${local.service_name}-execution-ssm-policy"
...
resource "aws_security_group" "ecs_tasks" {
  name_prefix = "${local.service_name}-ecs-tasks-"
  vpc_id      = var.vpc_id
...
resource "aws_lb" "main" {
  name               = substr(replace("${local.service_name}-alb", "_", "-"), 0, 32)
  internal           = false
...
resource "aws_appautoscaling_policy" "ecs_policy_cpu" {
  name               = "${local.service_name}-scale-cpu"
...
resource "aws_appautoscaling_policy" "ecs_policy_memory" {
  name               = "${local.service_name}-scale-memory"
  policy_type        = "TargetTrackingScaling"
  resource_id        = aws_appautoscaling_target.ecs_target.resource_id
  scalable_dimension = aws_appautoscaling_target.ecs_target.scalable_dimension
  service_namespace  = aws_appautoscaling_target.ecs_target.service_namespace
